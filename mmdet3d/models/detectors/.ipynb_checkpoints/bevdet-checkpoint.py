...
class BEVDet(CenterPoint):
    ...
    def __init__(self, img_view_transformer, img_bev_encoder_backbone,
                 img_bev_encoder_neck,only_train_depth=False,renderencoder=False, **kwargs):
    ...
    @force_fp32()
    def bev_encoder_render(self, x,msfeat,render_config,cam_params,occ2depth,img_to_ego_coor,view_transform_fill_all):
        # x = self.img_bev_encoder_backbone(x)
        x = self.img_bev_encoder_backbone(x,msfeat,render_config,cam_params,occ2depth,img_to_ego_coor,view_transform_fill_all)
        x,inter_occs=x
        x = self.img_bev_encoder_neck(x)
        if type(x) in [list, tuple]:
            x = x[0]
    ...
    def extract_feat(self, points, img, img_metas,fuse_self_args=None, **kwargs):
        """Extract features from images and points."""
        # import pdb;pdb.set_trace()
        img_feats, depth = self.extract_img_feat(img, img_metas,fuse_self_args=fuse_self_args, **kwargs)
    ...
    def forward_test(self,
        ...
        for var, name in [(img_inputs, 'img_inputs'),
            ...
            if not isinstance(var, list):
                ...
                #################
        # import pdb;pdb.set_trace()
        # img_inputs=[[img_inputs[i][0:1] for i in range(len(img_inputs))]]
...
class BEVStereo4D(BEVDepth4D):
    ...
    def prepare_bev_feat(self, img, sensor2keyego, ego2global, intrin,
                         ...
                         k2s_sensor, extra_ref_frame,gt_depth,gt_hidden=None,gt_occ = None,mask_camera =None,img_metas=None,key_frame=True):
        if extra_ref_frame:
            stereo_feat = self.extract_stereo_ref_feat(img)
            return None, None, stereo_feat
        if self.renderencoder:
            x, stereo_feat,msfeat = self.image_encoder(img, stereo=True)
        else:
        ...
        bev_feat, depth = self.img_view_transformer(
            [x, sensor2keyego, ego2global, intrin, post_rot, post_tran, bda,
             mlp_input,gt_depth,gt_hidden,gt_occ,mask_camera,img_metas,key_frame], metas)
        if self.only_train_depth:
            return bev_feat, depth, stereo_feat
        if self.pre_process:
            bev_feat = self.pre_process_net(bev_feat)[0]
        if self.renderencoder:
            return (bev_feat,msfeat), depth, stereo_feat
        else:
            return bev_feat, depth, stereo_feat
    ...
    def extract_img_feat(self,
        ...
        if sequential:
            ...
            assert False
        # import pdb;pdb.set_trace()
        if fuse_self_args is None:
            
            imgs, sensor2keyegos, ego2globals, intrins, post_rots, post_trans, \
            bda, curr2adjsensor = self.prepare_inputs(img, stereo=not self.wo_stereo)
            # import pdb;pdb.set_trace()
            """Extract features of images."""
            bev_feat_list = []
            ...
            for fid in range(self.num_frame-1, -1, -1):
                ...
                if key_frame or self.with_prev:
                    ...
                    gt_depth=kwargs['gt_depth'] if 'gt_depth' in kwargs else None
                    gt_hidden=kwargs['gt_hidden'] if 'gt_hidden' in kwargs else None
                    voxel_semantics=kwargs['voxel_semantics'] if 'voxel_semantics' in kwargs else None
                    
                    mask_camera=kwargs['mask_camera'] if 'mask_camera' in kwargs else None
                    inputs_curr = (img, sensor2keyego, ego2global, intrin,
                                post_rot, post_tran, bda, mlp_input,
                                feat_prev_iv, curr2adjsensor[fid],
                                extra_ref_frame,gt_depth,gt_hidden,voxel_semantics,mask_camera,img_metas,key_frame)
                    if key_frame:
                    ...
                    if self.renderencoder:
                        bev_feat.append(inputs_curr)
            if self.only_train_depth:
                return [None], depth_key_frame
            if pred_prev:
                # Todo
                assert False
            ...
            if self.lift_attn_with_ori_feat_add:  
                fuse_args,self_bev_feat,fuse_self_round =fuse_self_args  
                input_, depth, tran_feat,bev_feat_args_last_phase= fuse_args
                self_bev_feat,bev_feat_list=self_bev_feat
                update_bev_feat,depth_key_frame=self.img_view_transformer.view_transform(input_, depth, tran_feat,self_bev_feat=self_bev_feat,
                                                                                     fuse_self_round=fuse_self_round,bev_feat_args_last_phase=bev_feat_args_last_phase)
            else:
                fuse_args,self_bev_feat =fuse_self_args  
                input_, depth, tran_feat= fuse_args
                self_bev_feat,bev_feat_list=self_bev_feat
                update_bev_feat,depth_key_frame=self.img_view_transformer.view_transform(input_, depth, tran_feat,self_bev_feat=self_bev_feat)
            if self.pre_process:
                update_bev_feat = self.pre_process_net(update_bev_feat)[0]
            bev_feat_list[-1]=update_bev_feat
        if self.renderencoder:
            bev_feat,msfeat,inputs_curr=zip(bev_feat_list)
        bev_feat = torch.cat(bev_feat_list, dim=1)
        # import pdb;pdb.set_trace()
        if self.renderencoder:

            x ,inter_occs= self.bev_encoder_render(bev_feat,msfeat,render_config=self.img_view_transformer.render_config,cam_params=inputs_curr,
                                        occ2depth=self.img_view_transformer.occ2depth,img_to_ego_coor=self.img_view_transformer.img_to_ego_coor,
                                        view_transform_fill_all=self.img_view_transformer.view_transform_fill_all2)
        else:
            x = self.bev_encoder(bev_feat)
        if self.lift_attn_with_ori_feat_add: 
            return[x,bev_feat_list],depth_key_frame
        
        else:
            if self.fuse_self and fuse_self_args is None:
                return[x,bev_feat_list],depth_key_frame
            elif self.fuse_self and fuse_self_args is not None:
                # import pdb;pdb.set_trace()
