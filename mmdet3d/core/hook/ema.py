...
class MEGVIIEMAHook(Hook):
    """Modified from EMAHook used in BEVDepth.

    Modified from https://github.com/Megvii-Base
    Detection/BEVDepth/blob/main/callbacks/ema.py.
    """

    def __init__(self, init_updates=0, decay=0.9990, resume=None, interval=-1, max_keep_ckpts=1000):
        super().__init__()
        ...
        self.saved_checkpoints = []  # 跟踪保存的检查点文件路径
    ...
    def save_checkpoint(self, runner):
        ...
        self._manage_checkpoints(save_path)
    ...
    def _manage_checkpoints(self, new_checkpoint_path):
        """Manage checkpoints to keep only the latest max_keep_ckpts files."""
        self.saved_checkpoints.append(new_checkpoint_path)
        if len(self.saved_checkpoints) > self.max_keep_ckpts:
            # Sort checkpoints by name (assuming names reflect time order)
            ...
            if os.path.exists(old_checkpoint):
                try:
                    ...
                    print(f"Error deleting checkpoint {old_checkpoint}: {e}")