...
class EgoPoseDatasetOcc3D(Dataset):
    def __init__(self, data_infos,dataset='nusc'):
        super(EgoPoseDatasetOcc3D, self).__init__()
    ...
    def get_global_pose(self, info, inverse=False):
        if 'lidarego2global' not in info:
            ...
            pose=info['lidarego2global']
        return pose

    def __getitem__(self, idx):
        info = self.data_infos[idx]
        if hasattr(info,'token'):
            ref_sample_token = info['token']
        else:
            ref_sample_token=None
