...
class Metric_mIoU():
    def __init__(self,
        ...
        self.max_d = max_d
        self.min_d = min_d
        self.dynamic_object_idx=dynamic_object_idx
    ...
    def add_batch(self,semantics_pred,semantics_gt,mask_lidar,mask_camera):
        ...
        _,_hist_occ= self.compute_mIoU((masked_semantics_pred!=self.num_classes-1).astype(np.float32), (masked_semantics_gt!=self.num_classes-1).astype(np.float32), 2)
        self.hist += _hist
    ...
    def count_miou(self):
        ...
        occ_class=['empty','occupied']
        mIoU_occ = self.per_class_iu(self.hist_occ)
        #occupied iou
        for ind_class in range(1,2):
            print(f'===> {occ_class[ind_class]} - IoU = ' + str(round(mIoU_occ[ind_class] * 100, 4)))
            
        # res['empty']=round(mIoU_occ[0] * 100, 2)
        res['occupied']=round(mIoU_occ[1] * 100, 2)
        #miou_D

        print(f'===> mIoU_D = ' + str(round(np.nanmean(mIoU[self.dynamic_object_idx]) * 100, 4)))
        res['mIoU_D'] =  round(np.nanmean(mIoU[self.dynamic_object_idx]) * 100, 2)
