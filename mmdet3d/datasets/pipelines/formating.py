    ...
    def __init__(
        ...
        meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img',
                   ...
                   'sequence_group_idx', 'curr_to_prev_lidar_rt', 'curr_to_prev_ego_rt',
                    ...
                    'prev_lidar_to_global_rt', 'sample_index','sample_idx', 'scene_name', 'curr', 'nuscenes_get_rt_matrix', 'aux_cam_params',
                   'affine_aug','end_of_sequence','curr_to_next_lidar_rt','next_lidar_to_global_rt','curr_to_next_ego_rt','next_occ_gt_path','prev_occ_gt_path'
...
class OccDefaultFormatBundle3D(DefaultFormatBundle3D):
    ...
    def __call__(self, results):
        ...
        if 'gt_occupancy' in results.keys() and results['gt_occupancy'] is not None:
            if type(results['gt_occupancy']) is list:
                results['gt_occupancy'] = tuple([DC(to_tensor(x), stack=True) for x in results['gt_occupancy']])
            else:
                results['gt_occupancy'] = DC(to_tensor(results['gt_occupancy']), stack=True)
