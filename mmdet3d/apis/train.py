...
import time
from mmdet3d.models.fbocc_utils.utils import CustomDistEvalHook
...
def train_detector(model,
    ...
    batch_sampler=None if 'sampler' not in cfg else cfg.sampler
    data_loaders = [
    ...
    if cfg.resume_from:
        ...
        if hasattr(cfg,'revise_keys') and cfg.revise_keys is not None:
            runner.load_checkpoint(
                cfg.load_from,revise_keys=cfg.revise_keys
            )
        else:
            runner.load_checkpoint(cfg.load_from)
    runner.run(data_loaders, cfg.workflow)
