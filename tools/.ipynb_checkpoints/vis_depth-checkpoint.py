...
def main():
    ...
    if args.launcher =='cpu':
        ...
        for epoch, data in enumerate(data_loader):
            # import pdb;pdb.set_trace()
            # if data['img_metas'][0].data[0][0]['sample_idx']!='0b1c4f8426554e81b723df9e4acf1983' and data['img_metas'][0].data[0][0]['sample_idx']!='05cf2a68a2cf4ff1838034f8ecae0c82':
            #     continue
            # for i in range(6):
            #     # result_i = np.uint8(result[i]*255)
            #     # color_map = cv2.applyColorMap(result_i, cv2.COLORMAP_JET)
            #     save_dir=osp.join('depth_vis', data['img_metas'][0].data[0][0]['scene_name'],data['img_metas'][0].data[0][0]['sample_idx'])
...
#######################################
            ...
            result=model(return_loss=False, rescale=True,return_raw_occ=True,vis_depth=True, **data)
            depth,occ,occ_weight_=result
            num_occs=[0]*17
            num_points=[0]*17
            for i in range(6):
                # # result_i = np.uint8(result[i]*255)
                # # color_map = cv2.applyColorMap(result_i, cv2.COLORMAP_JET)
                # save_dir=osp.join('depth_vis',str(vis_class), data['img_metas'][0].data[0][0]['scene_name'],data['img_metas'][0].data[0][0]['sample_idx'])
                # if not osp.exists(save_dir):
                #     mmcv.mkdir_or_exist(save_dir)
                # # cv2.imwrite(osp.join(save_dir,f'grad_cam_{i}.jpg'), color_map)
                ...
                # # import pdb;pdb.set_trace()
                # depth_i=depth[i]
                # depth_i=F.interpolate(depth_i.unsqueeze(0),size=img.shape[:2],mode='bilinear',align_corners=True).squeeze(0)
                # depth_range=torch.arange(1.0, 45.0, 0.5).to(depth_i.device)
                # depth_i=(depth_i*depth_range.unsqueeze(-1).unsqueeze(-1)).sum(0).detach().cpu().numpy()
                # depth_i=depth_i/depth_i.max()
                # depth_i=np.uint8(depth_i*255)
                # color_map = cv2.applyColorMap(depth_i, cv2.COLORMAP_JET)
                # color_map=cv2.cvtColor(color_map, cv2.COLOR_RGB2BGR)
                # cv2.imwrite(osp.join(save_dir,f'depth_cam_{i}.jpg'), color_map)
##########################
                # occ_i=occ[i]
                # occ_i=F.interpolate(occ_i.unsqueeze(0),size=img.shape[:2],mode='bilinear',align_corners=True).squeeze(0)
                # # occ_i=occ_i>occ_i.mean(0).unsqueeze(0)
                # occ_i=occ_i.detach().cpu().numpy()
                # occ_i=occ_i.sum(0)
                # occ_i=occ_i/occ_i.max()
                # occ_i=np.uint8(occ_i*255)
                # color_map = cv2.applyColorMap(occ_i, cv2.COLORMAP_JET)
                # color_map=cv2.cvtColor(color_map, cv2.COLOR_RGB2BGR)
                # cv2.imwrite(osp.join(save_dir,f'occ_cam_{i}.jpg'), color_map)
                ##########################
                semantic_i=data['gt_semantic_map'][0][0][i]
                # occ_i=occ[i]
                occ_i=occ_weight_[i]
                occ_i=F.interpolate(occ_i.unsqueeze(0),size=(256,704),mode='bilinear',align_corners=True).squeeze(0)
                # occ_i=occ_i>occ_i.mean(0).unsqueeze(0)
                occ_i=occ_i
                occ_i=occ_i.sum(0)
                # import pdb;pdb.set_trace()
                for j in range(17):
                    num_points[j]+=(semantic_i==j).sum().item()
                    num_occs[j]+=(occ_i*(semantic_i.to(occ_i)==j)).sum().item()

                # import pdb;pdb.set_trace()
            if epoch%100==0:
                print(f'num_occs:{num_occs}')
                print(f'num_points:{num_points}')
                print(f'occ_ratio:{[num_occs[j]/(num_points[j]+1) for j in range(17)]}')
                
                
            print(f'depth_cam_{i}.jpg saved')
            prog_bar.update()
            # except:
            #     prog_bar.update()
