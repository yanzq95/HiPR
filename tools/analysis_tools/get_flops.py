...
def construct_input(input_shape):
    rot = torch.eye(3).float().cuda().view(1, 1, 3, 3).expand(1,6,3,3)

    intrins = torch.ones(3).float().cuda().view(1,1, 3).expand(1,6,3)
    
    rots = torch.eye(4).float().cuda().view(1, 1, 4, 4).expand(1,6,4,4)
    input = dict(img_inputs=[[
        torch.ones(()).new_empty((1, 12, *input_shape)).cuda(), rot,
        intrins, rot, rot,intrins,
        torch.eye(3).float().cuda().view(1, 3, 3)
    ]],
    aux_cam_params=[[rots,rots]],
    adj_aux_cam_params=[[rots,rots]],
    img_metas=[[{
        'index':1,
        'sample_idx':1,
        'aux_cam_params': [rots,rots],
        'sequence_group_idx':1,
        'start_of_sequence':True,
        'curr_to_prev_ego_rt':rots[:,0],
    }]],
    return_loss=False
    )
    return input
...
def main():
    ...
    # input_shape=tuple([3]+list(input_shape))
