...
# +----------------------+----------+----------+----------+
# |        others        |  0.144   |  0.149   |  0.149   |
# |       barrier        |  0.505   |  0.531   |  0.543   |
# |       bicycle        |  0.356   |  0.366   |  0.367   |
# |         bus          |  0.709   |  0.778   |  0.809   |
# |         car          |  0.613   |  0.671   |  0.690   |
# | construction_vehicle |  0.386   |  0.447   |  0.466   |
# |      motorcycle      |  0.460   |  0.484   |  0.487   |
# |      pedestrian      |  0.548   |  0.565   |  0.572   |
# |     traffic_cone     |  0.404   |  0.409   |  0.416   |
# |       trailer        |  0.417   |  0.452   |  0.478   |
# |        truck         |  0.578   |  0.649   |  0.675   |
# |  driveable_surface   |  0.602   |  0.687   |  0.772   |
# |      other_flat      |  0.342   |  0.393   |  0.439   |
# |       sidewalk       |  0.331   |  0.395   |  0.451   |
# |       terrain        |  0.321   |  0.399   |  0.463   |
# |       manmade        |  0.576   |  0.616   |  0.653   |
# |      vegetation      |  0.515   |  0.600   |  0.655   |
# +----------------------+----------+----------+----------+
# |         MEAN         |  0.459   |  0.505   |  0.534   |
# +----------------------+----------+----------+----------+
# {'RayIoU': 0.4996796866852318, 'RayIoU@1': 0.45922640745068705, 'RayIoU@2': 0.5053430827457238, 'RayIoU@4': 0.5344695698592846}
...
occ_2d_out_channels=80
depth_gt=True

fill_value = 0.0                    
gt_period_end=-1   
mix_period_end=num_iters_per_epoch*6  
...
model = dict(
    ...
    ############################################
    curr_mode='replace',
    gt_period_end=gt_period_end,
    mix_period_end=mix_period_end,
    backward_projection=dict(
        type='BackwardProjection_HGR', 
        bev_h=200,
        bev_w=200,
        in_channels=occ_2d_out_channels,
        out_channels=occ_2d_out_channels,
        pc_range=[-40, -40, -1.0, 40, 40, 5.4],
        transformer=dict(
            type='BEVFormer',
            use_cams_embeds=False,
            embed_dims=occ_2d_out_channels,
            encoder=dict(
                type='HGR_encoder',
                use_bev_height_mask=True,
                num_layers=3,
                ...
                transformerlayers=dict(
                    ...
                    ffn_cfgs=dict(
                        type='FFN',
                        embed_dims=occ_2d_out_channels,
                        feedforward_channels=occ_2d_out_channels*4,
                        ffn_drop=0.0,
        ...
        positional_encoding=dict(
            ...
            num_feats=occ_2d_out_channels//2,
            row_num_embed=200,
            col_num_embed=200,
            )),
    fuse=dict(type="Add"),
...
train_pipeline = [
    ...
    dict(
        ...
        downsample=1, 
        grid_config=grid_config,
        load_semantic_map=load_point_semantic,
        fix_void=fix_void),
    dict(
        type="Lidar2EgoAndHeightmap",
        trans2ego=False,
        x_bounds=(-40, 40),
        y_bounds=(-40, 40),
        resolution=0.4,
    ...
    dict(
        type='LoadOccupancy_heightgt', 
        ignore_nonvisible=use_camera_visible_mask, 
        convert2metric=False,
        fix_void=fix_void,   
...
custom_hooks = [
    ...
    dict(
        type='FusionRateControlHeightHook',
        temporal_start_iter=gt_period_end,
        temporal_end_iter=mix_period_end,
    ),
    dict(
        type='GetTrainingIter'),
